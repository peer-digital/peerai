...
async def get_analytics_data(
    ...
    average_latency = sum(r.latency_ms for r in records) / total_requests if total_requests > 0 else 0
...
async def get_user_stats(db: Session) -> Dict:
    ...
    active_users = db.query(UsageRecord.user_id).distinct().filter(
    ...
    pro_users = db.query(APIKey).filter(
        APIKey.daily_limit > 1000,
        APIKey.daily_limit <= 10000
    ).count()
