...
class ModelOrchestrator:
    ...
    def _get_provider(self, provider_id: int) -> ModelProvider:
        ...
        provider = self.db.query(ModelProvider).filter(ModelProvider.id == provider_id).first()
        if not provider:
    ...
    def _get_default_model(self, model_type: str = "text") -> AIModel:
        ...
        model = (
            ...
            .filter(AIModel.model_type == model_type, AIModel.is_default == True)
            .first()
        )
        if not model:
    ...
    def _transform_request(self, model: AIModel, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Start with any model-specific configuration
        transformed_request = model.config.copy() if model.config else {}
        
        # Add the model name if the provider requires it
        if provider.name == "mistral":
            transformed_request["model"] = model.name
        ...
        for peer_param, value in request_data.items():
            if peer_param in mappings:
                ...
                if mapping.transform_function:
                    value = self._apply_transform(mapping.transform_function, value, request_data)
                
                transformed_request[provider_param] = value
        
        return transformed_request

    def _apply_transform(self, function_name: str, value: Any, request_data: Dict[str, Any]) -> Any:
    ...
    def _get_api_key(self, provider: ModelProvider) -> str:
        ...
        if not api_key:
            raise ValueError(f"API key not found for provider '{provider.name}'. Set the {env_var} environment variable.")
    ...
    async def call_model(
        ...
        try:
            # Get the model
            model = self._get_model(model_name) if model_name else self._get_default_model()
            provider = self._get_provider(model.provider_id)
            ...
            unified_response = self._transform_response(provider.name, model.name, result)
            ...
            if api_key:
                self._record_usage(
                    api_key=api_key,
                    model=model.name,
                    tokens_used=unified_response.get("usage", {}).get("total_tokens", 0),
    ...
    def _transform_response(self, provider_name: str, model_name: str, response: Dict[str, Any]) -> Dict[str, Any]:
