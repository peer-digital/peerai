...
async def test_get_system_settings(db_session, admin_user, auth_token, async_client):
    ...
    db_session.query.return_value.filter.return_value.first.side_effect = [admin_user, mock_settings]
...
def test_update_system_settings(db_session, admin_user, auth_token):
    ...
    db_session.query.return_value.filter.return_value.first.side_effect = [admin_user, None]
...
async def test_get_analytics(db_session, admin_user, auth_token, async_client):
    ...
    with patch("backend.services.analytics.get_analytics_data", new_callable=AsyncMock) as mock_get:
...
async def test_get_user_stats(db_session, admin_user, auth_token, async_client):
    ...
    with patch("backend.services.analytics.get_user_stats", new_callable=AsyncMock) as mock_get:
...
async def test_export_analytics_data(db_session, admin_user, auth_token, async_client):
    ...
    with patch("backend.services.analytics.export_analytics_data", new_callable=AsyncMock) as mock_export_fn:
...
async def test_get_analytics_data(db_session, admin_user, admin_token, async_client):
    ...
    with patch("backend.services.analytics.get_analytics_data", new_callable=AsyncMock) as mock_analytics:
    ...
    assert all(key in data["usage"] for key in ["requests", "dates", "success_rate", "avg_latency"])
...
async def test_get_user_stats(db_session, admin_user, admin_token, async_client):
    ...
    with patch("backend.services.analytics.get_user_stats", new_callable=AsyncMock) as mock_stats_fn:
    ...
    assert response.status_code == 200
    data = response.json()
    assert all(key in data for key in ["total_users", "active_users", "new_users_last_week"])
...
@pytest.mark.asyncio
async def test_get_analytics_by_timeframe(db_session, admin_user, admin_token, async_client):
    ...
    for timeframe in timeframes:
        mock_data = {
            "usage": {
                "total_requests": 100 * (1 if timeframe == "24h" else 7 if timeframe == "7d" else 30),
                "total_tokens": 5000 * (1 if timeframe == "24h" else 7 if timeframe == "7d" else 30)
            }
        }
...
@pytest.mark.asyncio
async def test_export_analytics_data_formats(db_session, admin_user, admin_token, async_client):
...
@pytest.mark.asyncio
async def test_analytics_access_control(db_session, admin_user, user_token, admin_token, async_client):
