...
from tests.test_db_integration import db_session
...
async def test_complete_auth_flow(async_client):
    ...
    login_response = await async_client.post(
        "/api/v1/auth/login",
        data={"username": "new@peerai.se", "password": "newpass123"},
    )
...
async def test_api_key_management(db_session, test_user, user_token, async_client):
    ...
    response = await async_client.get(
        "/api/v1/auth/api-keys", headers={"Authorization": f"Bearer {user_token}"}
    )
...
async def test_error_handling(db_session, test_api_key, async_client):
    ...
    db_session.query.return_value.filter.return_value.count.return_value = (
        test_api_key.minute_limit + 1
    )
...
async def test_websocket_connection(test_api_key, async_client):
    ...
    async with async_client.websocket_connect(
        ...
        await websocket.send_json({"prompt": "Test streaming", "mock_mode": True})
