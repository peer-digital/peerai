...
from backend.schemas.rbac import TeamCreate
...
def test_admin(db_session: Session):
    """Create a test admin"""
    # Generate unique email for each test run
    unique_email = f"test_admin_{int(time.time())}@example.com"
    admin = User(
        email=unique_email,
...
def test_team(db_session: Session, test_admin: User):
    """Create a test team"""
    team = Team(name=test_team_data["name"], created_by_id=test_admin.id)
    db_session.add(team)
    db_session.commit()
...
def test_create_team(db_session: Session, test_admin: User):
    ...
    team = RBACService.create_team(
        db_session, TeamCreate(name=test_team_data["name"]), test_admin
    )
...
def test_add_user_to_team(
    db_session: Session, test_admin: User, test_user: User, test_team: Team
):
    """Test adding user to team"""
    updated_user = RBACService.add_user_to_team(
        db_session, test_user.id, test_team.id, test_admin
    )
...
def test_update_user_role(db_session: Session, test_admin: User, test_user: User):
    ...
    updated_user = RBACService.update_user_role(
        db_session, test_user.id, Role.USER_ADMIN, test_admin
    )
...
def test_get_team_members(
    ...
    members = RBACService.get_team_members(db_session, test_team.id, test_admin)
...
def test_remove_user_from_team(
    ...
    updated_user = RBACService.remove_user_from_team(
        db_session, test_user.id, test_admin
    )
    assert updated_user.team_id is None


def test_unauthorized_team_access(
    db_session: Session, test_user: User, test_team: Team
):
...
def test_unauthorized_role_update(db_session: Session, test_user: User):
    ...
    with pytest.raises(Exception) as exc_info:
        RBACService.update_user_role(
            db_session, test_user.id, Role.USER_ADMIN, test_user
        )
    assert "Not authorized" in str(exc_info.value)
